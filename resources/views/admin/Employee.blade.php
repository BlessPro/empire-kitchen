<x-layouts.app>
  ...
  <main class="ml-[280px] mt-[100px] flex-1 bg-[#F9F7F7] min-h-screen">
    <div class="p-6">
      <div class="flex items-center justify-between mb-6">
       ...
       <a href="addemployee">
            <button class="px-6 py-2 text-semibold text-[15px] text-white rounded-full bg-fuchsia-900 hover:bg-[#F59E0B]">
                            + Add Employee
                        </button>
                    </a>
      </div>
            ...
            <tbody class="divide-y divide-gray-100">
              @forelse($employees as $e)
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-3">
                    @if($e->phone)
                      <a href="tel:{{ $e->phone }}" class="text-fuchsia-900 hover:underline">{{ $e->phone }}</a>
                    @endif
                  </td>
                  <td class="px-6 py-3">
                    @if($e->email)
                      <a href="mailto:{{ $e->email }}" class="text-fuchsia-900 hover:underline">{{ $e->email }}</a>
                    @endif
                  </td>
   ...
   class="block w-full text-left px-4 py-2 hover:bg-gray-50">Edit Employee</a>
                      ...
                      <button class="block w-full text-left px-4 py-2 hover:bg-gray-50"
                              onclick="openBiodataModal(this)"
                              data-fullname="{{ $e->name }}"
                              data-designation="{{ $e->designation }}"
                              data-phone="{{ $e->phone }}"
                              data-email="{{ $e->email }}"
                              data-nationality="{{ $e->nationality }}"
                              data-dob="{{ $e->dob }}"
                              data-hometown="{{ $e->hometown }}"
                              data-language="{{ $e->language }}"
                              data-address="{{ $e->address }}"
                              data-gps="{{ $e->gps }}"
                              data-nok="{{ $e->next_of_kin }}"
                              data-relation="{{ $e->relation }}"
                              data-nokphone="{{ $e->nok_phone }}"
                              data-bank="{{ $e->bank }}"
  ...
  {{-- Biodata Modal --}}
  <div id="biodataModal"
       class="fixed inset-0 z-[140] hidden items-center justify-center bg-black/40">
    <div class="w-full max-w-2xl p-6 bg-white rounded-2xl shadow-xl" onclick="event.stopPropagation()">
      <div class="flex items-center justify-between mb-4">
        ...
        <button type="button" onclick="closeBiodataModal()" class="text-gray-500 hover:text-gray-700">✕</button>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-sm">
        ...
        <div><span class="text-gray-500">Nationality:</span> <span id="bio_nationality" class="ml-2"></span></div>
        ...
        <div><span class="text-gray-500">Language:</span> <span id="bio_language" class="ml-2"></span></div>
        <div class="md:col-span-2"><span class="text-gray-500">Address:</span> <span id="bio_address" class="ml-2"></span></div>
        <div><span class="text-gray-500">GPS:</span> <span id="bio_gps" class="ml-2"></span></div>
        <div><span class="text-gray-500">Commencement Date:</span> <span id="bio_commenced" class="ml-2"></span></div>

        <div class="mt-2 md:col-span-2 border-t pt-3">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3">
            <div><span class="text-gray-500">Next of Kin:</span> <span id="bio_nok" class="ml-2"></span></div>
            <div><span class="text-gray-500">Relation:</span> <span id="bio_relation" class="ml-2"></span></div>
            <div><span class="text-gray-500">NOK Phone:</span> <span id="bio_nokphone" class="ml-2"></span></div>
          </div>
        </div>

        <div class="mt-2 md:col-span-2 border-t pt-3">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-3">
            <div><span class="text-gray-500">Bank:</span> <span id="bio_bank" class="ml-2"></span></div>
            <div><span class="text-gray-500">Branch:</span> <span id="bio_branch" class="ml-2"></span></div>
            <div><span class="text-gray-500">Account #:</span> <span id="bio_account" class="ml-2"></span></div>
          </div>
        </div>
      </div>

      <div class="mt-6 flex justify-end">
        <button type="button" onclick="closeBiodataModal()" class="px-4 py-2 border rounded-lg">Close</button>
      </div>
    </div>
  </div>
