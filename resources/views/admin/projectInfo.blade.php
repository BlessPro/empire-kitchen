...
<details open class="group">
  <summary class="flex items-center justify-between gap-2 px-5 py-4 list-none cursor-pointer">
    ...
    <svg class="w-5 h-5 text-gray-500 transition group-open:rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor">
...
@endforeach
    ...
    <div class="mt-5">
      <div class="flex items-center justify-between mb-1 text-xs text-gray-600">
        <span>Completion</span><span><span id="phase-done">{{ $done }}</span>/{{ $total }}</span>
      </div>
      <div class="relative h-[6px] w-full overflow-hidden rounded-full bg-gray-100"
        ...
        <div id="phase-bar" class="absolute left-0 top-0 h-[6px] rounded-full bg-[#1DC76B]" style="width: {{ $pct }}%"></div>
      </div>
      <div id="phase-pct" class="mt-1 text-xs font-medium text-right text-emerald-700">{{ $pct }}%</div>
    </div>
  </div>
...
@if (!empty($project->products))
  <div class="flex items-center gap-2 px-4 py-3 mt-3 overflow-x-auto">
    ...
    @foreach ($project->products as $p)
      ...
      @endphp
      <a
        href="{{ request()->fullUrlWithQuery(['product_id' => $p['id']]) }}"
...
@php
  ...
  $activeProductId = collect($project->products ?? [])->firstWhere('active', true)['id'] ?? null;
  $activeType      = $project->selected_product_type ?? '—';
@endphp

<a href="#"
   id="btn-edit-product-type"
...
<a href="#"
   ...
   class="{{ $editBtn }} {{ $activeProductId ? '' : 'pointer-events-none opacity-50' }}"
   data-product-id="{{ $activeProductId }}"
   data-current-label="{{ $project->finish['label'] ?? '—' }}"
   data-current-color="{{ $project->finish['color'] ?? '' }}"
   data-current-image="{{ $project->finish['image_url'] ?? '' }}">
  Edit
</a>
...
<div class="flex flex-wrap items-center gap-2 px-4 py-3 text-sm">
  <span id="finish-label" class="{{ $chip }}">{{ $project->finish['label'] }}</span>
  <span class="{{ $chip }}">Color: <span
                                        class="ml-1 font-medium">{{ $project->finish['color'] }}</span></span>
  @if (!empty($project->finish['color']))
    <span id="finish-color"
          class="inline-flex w-8 h-5 rounded-md ring-1 ring-gray-300"
  ...
  @if (!empty($project->finish['image_url']))
    <span id="finish-image"
          class="inline-block w-10 h-5 bg-center bg-cover rounded-md ring-1 ring-gray-300"
          style="background-image: url('{{ $project->finish['image_url'] }}')"></span>
  @else
...
<a href="#"
   ...
   class="{{ $editBtn }} {{ $activeProductId ? '' : 'pointer-events-none opacity-50' }}"
   data-product-id="{{ $activeProductId }}"
   data-current-label="{{ $project->worktop['label'] ?? '—' }}"
   data-current-color="{{ $project->worktop['color'] ?? '' }}"
   data-current-image="{{ $project->worktop['image_url'] ?? '' }}">
  Edit
</a>
...
<div class="flex flex-wrap items-center gap-2 px-4 py-3 text-sm">
  <span id="worktop-label" class="{{ $chip }}">{{ $project->worktop['label'] }}</span>

  @if (!empty($project->worktop['color']))

               <span class="{{ $chip }}">Color: <span
                                        class="ml-1 font-medium">{{ $project->worktop['color'] }}</span></span>
    <span id="worktop-color"
          class="inline-flex w-8 h-5 rounded-md ring-1 ring-gray-300"
  ...
  @if (!empty($project->worktop['image_url']))
    <span id="worktop-image"
          class="inline-block w-10 h-5 bg-center bg-cover rounded-md ring-1 ring-gray-300"
          style="background-image: url('{{ $project->worktop['image_url'] }}')"></span>
  @else
...
<a href="#"
   ...
   class="{{ $editBtn }} {{ $activeProductId ? '' : 'pointer-events-none opacity-50' }}"
   data-product-id="{{ $activeProductId }}"
   data-current-sink="{{ $project->sink_tap['bowl'] ?? '—' }}"
   data-current-tap="{{ $project->sink_tap['handle'] ?? '—' }}"
   data-current-color="{{ $project->sink_tap['color'] ?? '' }}"
   data-current-image="{{ $project->sink_tap['image_url'] ?? '' }}">
  Edit
</a>
...
<div class="flex flex-wrap items-center gap-2 px-4 py-3 text-sm">
  <span id="sink-bowl-label" class="{{ $chip }}">{{ $project->sink_tap['bowl'] }}</span>

  @if (!empty($project->sink_tap['color']))
    <span id="sink-color" class="inline-flex w-8 h-5 rounded-md ring-1 ring-gray-300"
          style="background: {{ e($project->sink_tap['color']) }}"></span>
          <span class="{{ $chip }}">Color: <span id="sinktap-color-hex"
  ...
  <span id="sink-handle-label" class="{{ $chip }}">{{ $project->sink_tap['handle'] }}</span>

  @if (!empty($project->sink_tap['image_url']))
    <span id="sink-image" class="inline-block w-10 h-5 bg-center bg-cover rounded-md ring-1 ring-gray-300"
          style="background-image: url('{{ $project->sink_tap['image_url'] }}')"></span>
  @else
              ...
              <li class="flex items-center justify-between">
                <div class="flex items-center min-w-0 gap-3">
                  @if (!empty($it['thumb_url']))
                    <img src="{{ $it['thumb_url'] }}" alt="" class="object-cover w-12 h-12 rounded-md">
                  @else
                    <div class="w-12 h-12 bg-gray-100 rounded-md ring-1 ring-gray-300"></div>
                  @endif
                  <div class="min-w-0">
                    <div class="text-sm text-gray-800 truncate">{{ $it['name'] ?? '—' }}</div>
                    <div class="text-xs text-gray-500">
                      @if (!empty($it['uploaded_at_label']))
                        Uploaded on {{ $it['uploaded_at_label'] }}
                      @endif
                      @if (!empty($it['size_label']))
                        <span class="mx-1">•</span>{{ $it['size_label'] }}
                      @endif
                    </div>
                  </div>
                </div>
                <div class="flex items-center gap-3 shrink-0">
                  @if (!empty($it['url']))
                    <a href="{{ $it['url'] }}" target="_blank" class="text-gray-700 hover:text-gray-900" title="View">
                      <iconify-icon icon="mdi:eye-outline" class="text-xl"></iconify-icon>
                    </a>
                  @endif
                  @if (!empty($it['download_url']))
                    <a href="{{ $it['download_url'] }}" download class="text-[#5A0562] hover:text-[#43044a]" title="Download">
...
<div id="product-type-modal" class="fixed inset-0 z-[60] hidden">
  ...
  <div class="absolute left-1/2 top-1/2 w-[420px] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl">
    ...
    <form id="pt-form" class="p-5 space-y-4">
      ...
      <div>
        ...
        <select id="pt-select"
                class="w-full border-gray-300 rounded-lg focus:ring-fuchsia-600 focus:border-fuchsia-600" required>
      ...
      <div class="flex items-center justify-end gap-2 pt-2">
        ...
        <button type="submit" class="px-3 py-1.5 rounded-lg bg-fuchsia-900 text-white text-sm w-full">Save</button>
      </div>
    </form>
  </div>
</div>
...
<div id="finish-modal" class="fixed inset-0 z-[60] hidden">
  ...
  <div class="absolute left-1/2 top-1/2 w-[600px] max-w-[94vw] -translate-x-1/2 -translate-y-1/2 rounded-[20px] bg-white shadow-xl">
    {{-- Header --}}
      ...
      <div>
        ...
        <div class="relative">
          <select id="finish-type"
                  ...
                  required>
            @php
              $finishOptions = ['High Gloss','Matte','Satin','Textured','Laminate','Painted','Veneer','Others'];
            @endphp
          ...
          <span class="absolute text-gray-400 -translate-y-1/2 pointer-events-none right-3 top-1/2">
            <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path d="m6 9 6 6 6-6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </span>
        </div>
      </div>
      ...
      <div class="grid gap-6 md:grid-cols-2">
        ...
        <div>
          ...
          <div class="flex items-center">
            {{-- visible color chip --}}
            <span id="finish-color-chip" class="inline-block w-6 h-6 mr-3 rounded-md ring-1 ring-gray-300"></span>
          ...
          <p class="mt-2 text-xs text-gray-500">Paste a hex like <code>#112233</code> or click the chip to pick.</p>
        </div>
        ...
        <div>
          ...
          <input type="file" id="finish-image-input" accept="image/png,image/jpeg,image/webp" class="hidden">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="relative">
              ...
              <button type="button" id="finish-image-clear"
                      ...
                      title="Remove">
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path d="M3 6h18M8 6v12m8-12v12M5 6l1 14a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2l1-14" stroke-width="1.5" stroke-linecap="round"/>
                  <path d="M10 6V4a2 2 0 0 1 2-2h0a2 2 0 0 1 2 2v2" stroke-width="1.5" stroke-linecap="round"/>
                </svg>
              </button>
            </div>
          </div>
...
<div id="glass-modal" class="fixed inset-0 z-[60] hidden">
  ...
  <div class="absolute left-1/2 top-1/2 w-[480px] max-w-[94vw] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl">
        ...
        <div class="relative">
          <select id="glass-select"
            ...
            @php
              ...
              $glassOptions = ['Clear', 'Frosted', 'Tinted', 'Smoked', 'Textured', 'Reeded', 'Laminated', 'Others'];
            @endphp
    ...
    <form id="worktop-form" class="px-6 py-5 space-y-6" enctype="multipart/form-data">
      ...
      <div>
        ...
        <div class="relative">
          <select id="worktop-type"
                  ...
                  required>
            @php
              $worktopOptions = ['Granite','Quartz','Marble','Solid Surface','Laminate','Wood','Porcelain','Others'];
            @endphp
      ...
      <div class="grid gap-6 md:grid-cols-2">
        ...
        <div>
          ...
          <div class="flex items-center">
            <span id="worktop-color-chip" class="inline-block w-6 h-6 mr-3 rounded-md ring-1 ring-gray-300"></span>
            <div class="relative flex-1">
        ...
        <div>
          ...
          <input type="file" id="worktop-image-input" accept="image/png,image/jpeg,image/webp" class="hidden">
...
<div id="sinktap-modal" class="fixed inset-0 z-[60] hidden">
  ...
  <div class="absolute left-1/2 top-1/2 w-[600px] max-w-[94vw] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl">
    <div class="flex items-center justify-between px-6 py-4 border-b">
    ...
    <form id="sinktap-form" class="px-6 py-5 space-y-6" enctype="multipart/form-data">
        ...
        <div>
          ...
          <div class="relative">
            <select id="sinktap-sink"
                    ...
                    required>
              @php
                $sinkOptions = ['Single Bowl','Double Bowl','Farmhouse','Undermount','Topmount','Integrated','Others'];
              @endphp
            ...
            <span class="absolute text-gray-400 -translate-y-1/2 pointer-events-none right-3 top-1/2">
              <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="m6 9 6 6 6-6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
            </span>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="relative">
            <select id="sinktap-tap"
                    ...
                    required>
              @php
                $tapOptions = ['Single Lever','Dual Handle','Pull-Out','Pull-Down','Bridge','Wall-Mounted','Knob','Bar Handle','Others'];
              @endphp
      ...
      <div class="grid gap-6 md:grid-cols-2">
        ...
        <div>
          <label class="block mb-2 text-sm font-medium text-gray-800">Color</label>
          <div class="flex items-center">
            <span id="sinktap-color-chip" class="inline-block w-6 h-6 mr-3 rounded-md ring-1 ring-gray-300"></span>
            <div class="relative flex-1">
        ...
        <div>
          ...
          <input type="file" id="sinktap-image-input" accept="image/png,image/jpeg,image/webp" class="hidden">
...
<div id="acc-edit-modal" class="fixed inset-0 z-[70] hidden">
  ...
  <div class="absolute left-1/2 top-1/2 w-[860px] max-w-[95vw] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl overflow-hidden">
    <div class="flex items-center justify-between px-6 py-4 border-b">
    ...
    <div id="acc-edit-root"
         x-data="accEditEditor({
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          <button type="button" class="px-3 py-1.5 rounded-lg border text-sm hover:bg-gray-50" @click="addRow()">+ Add row</button>
        </div>
      </div>
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody>
            <template x-if="!rows.length">
              <tr><td colspan="6" class="px-3 py-6 text-center text-gray-500">No accessories yet.</td></tr>
            </template>
              ...
              <tr class="border-t">
                ...
                <td class="px-3 py-2">
                  <select class="w-full px-2 py-1.5 border rounded-lg"
                          x-model.number="row.accessory_id"
                          @change="onAccessoryChange(i)">
                    ...
                    <template x-for="t in (typesMap[row.accessory_id] || [])" :key="t">
                      <label class="inline-flex items-center gap-1">
                        <input type="radio" :name="`type_${row._key}`" :value="t"
                               class="w-4 h-4 text-fuchsia-700"
                               :checked="row.type === t"
                               @change="row.type = t">
                        <span x-text="t"></span>
                      </label>
                ...
                <!-- Qty -->
                <td class="px-3 py-2">
                  <input class="w-20 px-2 py-1.5 border rounded-lg" type="number" min="0" x-model.number="row.quantity">
                </td>

                <!-- Notes -->
                <td class="px-3 py-2">
                  <input class="w-full px-2 py-1.5 border rounded-lg" type="text" x-model="row.notes" placeholder="Optional">
                </td>
      ...
      <div class="sticky bottom-0 left-0 right-0 px-6 py-4 -mx-6 -mb-4 border-t bg-white/90 backdrop-blur">
        <div class="flex items-center justify-between gap-3">
          ...
          <div class="flex gap-2">
            <button type="button" id="acc-edit-cancel" class="px-4 py-2 border rounded-lg">Cancel</button>
            <button type="button" class="px-6 py-2 rounded-lg bg-[#4B0066] text-white" @click="save()">Save changes</button>
          </div>
        </div>
      </div>
...
function accEditEditor({ listUrlTpl, bulkUrlTpl, csrf }) {
  return {
    ...
    async open(pid, pname) {
      ...
      this.catalog  = cat.map(x => ({ id: Number(x.id), name: String(x.name ?? '') }));
      ...
      this.rows = items.map(it => {
        ...
        if (!catalogIds.has(aid)) {
          this.catalog.push({ id: aid, name: `(Unknown #${aid})` });
          catalogIds.add(aid);
        }
    ...
    addRow(){
      this.rows.push({ _key:'e-'+Date.now()+Math.random(), accessory_id:'', type:'', size:'', quantity:1, notes:'', _deleted:false });
    },
    ...
    _esc(s){ return (s??'').toString().replace(/[&<>"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]||c)); }
  };
}
...
<div id="acc-add-modal" class="fixed inset-0 z-[70] hidden">
  ...
  <div class="absolute left-1/2 top-1/2 w-[720px] max-w-[94vw] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-xl overflow-hidden">
    <div class="flex items-center justify-between px-6 py-4 border-b">
    ...
    <div id="acc-add-root"
      ...
      <div class="flex items-center justify-between">
        <div class="text-sm text-gray-600" x-text="rows.length ? `${rows.length} to add` : 'Add one or more appliances'"></div>
        <div class="flex gap-2">
          <button type="button" class="px-3 py-1.5 rounded-lg border text-sm hover:bg-gray-50" @click="openNewAccessory()">+ New accessory</button>
          <button type="button" class="px-3 py-1.5 rounded-lg border text-sm hover:bg-gray-50" @click="addRow()">+ Add row</button>
        </div>
      </div>

      <template x-if="rows.length === 0">
        <div class="p-4 text-sm text-gray-500 border border-dashed rounded-xl">No rows yet. Click “Add row”.</div>
      </template>
      ...
      <template x-for="(row, i) in rows" :key="row._key">
        <div class="p-4 space-y-4 border rounded-2xl">
          ...
          <div>
            ...
            <template x-if="sizes.length">
              <select class="w-full px-3 py-2 mt-1 border border-gray-300 rounded-xl" x-model="row.size">
                <option value="">Select size</option>
          ...
          <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div>
              ...
              <input type="number" min="0" class="w-full px-3 py-2 mt-1 border border-gray-300 rounded-xl"
          ...
          <div class="flex items-center justify-end">
            <button type="button" class="text-sm text-red-600 hover:underline" @click="remove(i)">Remove</button>
          </div>
        </div>
      </template>
      ...
      <div class="sticky bottom-0 left-0 right-0 px-6 py-4 -mx-6 -mb-4 border-t bg-white/90 backdrop-blur">
        <div class="flex items-center justify-end gap-3">
          <button type="button" id="acc-add-cancel" class="px-4 py-2 border rounded-lg">Cancel</button>
          <button type="button" class="px-6 py-2 rounded-lg bg-[#4B0066] text-white" @click="save()">Add</button>
        </div>
        <p id="acc-add-error" class="hidden mt-2 text-sm text-red-600"></p>
      </div>

      {{-- NEW ACCESSORY MINI MODAL --}}
      <div x-show="modal.open" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center bg-black/30">
        <div class="w-full max-w-md p-5 bg-white shadow-xl rounded-2xl">
          <div class="flex items-center justify-between mb-3">
            <h4 class="font-semibold">New Accessory</h4>
            <button type="button" @click="closeNewAccessory()" class="text-gray-500 hover:text-gray-700">✕</button>
          </div>

          <div class="space-y-3">
            <div>
              <label class="block text-sm font-medium">Name</label>
              <input type="text" class="w-full px-3 py-2 border rounded-lg" x-model="modal.form.name" placeholder="e.g. Hood" />
            ...
            <div class="flex justify-end gap-2 pt-2">
              <button type="button" @click="closeNewAccessory()" class="px-3 py-2 border rounded-lg">Cancel</button>
              <button type="button" @click="saveNewAccessory()" class="px-4 py-2 text-white rounded-lg bg-fuchsia-900">Save</button>
            </div>
...
<script>
function accAddEditor({ listUrlTpl, attachUrlTpl, createUrl, csrf }) {
  return {
    productId: null,
    catalog: [], sizes: [], typesMap: {},
    rows: [],
    ...
    async open(pid, pname) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          this._err('Could not load accessories. Please refresh.');
          this.catalog = []; this.sizes = []; this.typesMap = {};
          this.rows = [{ _key: 'r-'+Date.now(), accessory_id: '', type:'', size:'', quantity:1, notes:'' }];
          return;
        }
        data = await res.json();
      } catch (e) {
      ...
      if (debug) { debug.classList.remove('hidden'); debug.textContent = `Loaded: ${this.catalog.length} accessories • ${this.sizes.length} sizes`; }
    },

    addRow(){ this.rows.push({ _key:'r-'+Date.now()+Math.random(), accessory_id:'', type:'', size:'', quantity:1, notes:'' }); },
    ...
    openNewAccessory(){ this.modal.open = true; this.modal.form = { name:'', types_csv:'' }; this.errors = {}; this.flash=''; },
    closeNewAccessory(){ this.modal.open = false; },
    ...
    async saveNewAccessory(){
      ...
      const res = await fetch(createUrl, {
        method:'POST', credentials:'same-origin',
        headers:{ 'Accept':'application/json','Content-Type':'application/json','X-CSRF-TOKEN': csrf },
        body: JSON.stringify({ name: this.modal.form.name, types_csv: this.modal.form.types_csv })
      });
      if (!res.ok) {
        if (res.status === 422) {
          ...
          this.errors = { name: 'Failed to save accessory.' };
        }
        return;
      }
      const data = await res.json(); // {id,name,types[]}
      this.catalog = [...this.catalog, { id: data.id, name: data.name }];   // new reference for reactivity
      this.typesMap = { ...this.typesMap, [data.id]: (data.types || []) }; // new reference
    ...
    async save(){
      ...
      const payload = { items: this.rows.map(r => ({
    ...
    _err(msg){ const el = document.getElementById('acc-add-error'); if (el){ el.textContent = msg; el.classList.remove('hidden'); } },
    _esc(s){ return (s??'').toString().replace(/[&<>"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]||c)); }
  };
}
...
(function () {
  ...
  function openModal(productId, currentType) {
    ...
    Array.from(sel.options).forEach(o => { o.selected = (o.value === currentType); });
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch(`{{ route('admin.products.updateType', ['product' => '__ID__']) }}`
...
(function(){
  ...
  colorIn.addEventListener('input', () => { colorHx.value = colorIn.value; });
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    const productId = hidId.value;
    if (!productId) { errEl.textContent = 'No product selected.'; errEl.classList.remove('hidden'); return; }
    ...
    try {
      const res = await fetch(`{{ route('admin.products.updateFinish', ['product' => '__ID__']) }}`
                                .replace('__ID__', encodeURIComponent(productId)), {
        method: 'POST',
        credentials: 'same-origin',
...
(function(){
  ...
  form?.addEventListener('submit', async (e)=>{
    ...
    try {
      const res = await fetch(`{{ route('admin.products.updateGlassDoor', ['product' => '__ID__']) }}`
                                .replace('__ID__', encodeURIComponent(productId)), {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrf },
        body: JSON.stringify({ glass_door_type: v })
      });
      if (!res.ok) {
        const txt = await res.text();
...
(function(){
  ...
  function validHex(v){ return /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test((v||'').trim()); }
  function setChip(color){ colorChip.style.backgroundColor = color || '#cccccc'; }
  function setPreview(url){
    if (url) {
      preview.style.backgroundImage = `url('${url}')`;
      clearBn.classList.remove('hidden');
  ...
  form?.addEventListener('submit', async (e)=>{
    ...
    try {
      const res = await fetch(`{{ route('admin.products.updateWorktop', ['product' => '__ID__']) }}`
                                .replace('__ID__', encodeURIComponent(productId)), {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
...
(function(){
  ...
  // color sync
  colorChip.addEventListener('click', ()=> colorIn.click());
  colorIn.addEventListener('input', ()=> { setChip(colorIn.value); colorHex.value = colorIn.value; });
  colorHex.addEventListener('input', ()=> { const v = colorHex.value.trim(); if (validHex(v)) setChip(v); });
  ...
  form?.addEventListener('submit', async (e)=>{
    ...
    try {
      const res = await fetch(`{{ route('admin.products.updateSinkTap', ['product' => '__ID__']) }}`
                                .replace('__ID__', encodeURIComponent(productId)), {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'X-CSRF-TOKEN': csrf, 'Accept': 'application/json' },
      ...
      if (colorEl) {
        if (st.color) { colorEl.style.background = st.color; colorEl.classList.remove('hidden'); }
        else { colorEl.style.background = ''; colorEl.classList.add('hidden'); }
      }
      if (imgEl) {
        if (st.image_url) { imgEl.style.backgroundImage = `url('${st.image_url}')`; imgEl.classList.remove('hidden'); }
        else { imgEl.style.backgroundImage = ''; imgEl.classList.add('hidden'); }
      }
...
function accAddEditor({ listUrlTpl, attachUrlTpl, createUrl, csrf }) {
  return {
    ...
    async saveNewAccessory(){
      ...
      this.catalog.push({ id: data.id, name: data.name });
      this.typesMap[data.id] = data.types || [];
      this.flash = 'Accessory created.';
      // optional: auto-select in the last row
      const last = this.rows[this.rows.length-1];
      if (last) { last.accessory_id = data.id; last.type = ''; }
      setTimeout(()=> this.closeNewAccessory(), 600);
    },
    ...
    cb.addEventListener('change', async (e) => {
      ...
      try {
        const res = await fetch(el.dataset.url, {
          ...
          body: JSON.stringify({ checked: el.checked ? 1 : 0 }),
        });
