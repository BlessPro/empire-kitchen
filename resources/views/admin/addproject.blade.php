...
{{--
@php
  // sizes can be global or per-accessory if you later wire a sizes table
  $sizes = ['45cm','50cm','54cm','60cm','70cm','80cm','90cm'];
@endphp
...
<div class="space-y-2">
  @foreach ($appliances as $a)
    @php
      $row = old('accessories.'.$a->id, []); // keep user’s inputs on validation error
      $wasSelected = isset($row['selected']) && $row['selected'];
    @endphp

    <div x-data="{ on: @js($wasSelected) }" class="p-3 bg-white border rounded-xl">
      <label class="flex items-center gap-3 cursor-pointer">
        <input
          type="checkbox"
          x-model="on"
          name="accessories[{{ $a->id }}][selected]"
          value="1"
          class="shrink-0"
        >
      ...
      <!-- Show size/type only when selected -->
      <div x-show="on" x-cloak class="grid grid-cols-1 gap-3 mt-3 md:grid-cols-3">
        <!-- Size -->
        <div>
          <label class="block mb-1 text-sm text-gray-700">Size</label>
          <select
            name="accessories[{{ $a->id }}][size]"
        ...
        <!-- Type -->
        <div class="md:col-span-2">
          <label class="block mb-1 text-sm text-gray-700">Product Type</label>
          <div class="flex gap-4 items-center h-[42px]">
            <label class="inline-flex items-center gap-2">
              <input type="radio"
                     name="accessories[{{ $a->id }}][type]"
                     value="Inbuilt"
                     :required="on"
                     {{ ($row['type'] ?? '') === 'Inbuilt' ? 'checked' : '' }}>
              <span class="text-sm">Inbuilt</span>
            </label>
            <label class="inline-flex items-center gap-2">
              <input type="radio"
                     name="accessories[{{ $a->id }}][type]"
                     value="Freestanding"
                     :required="on"
                     {{ ($row['type'] ?? '') === 'Freestanding' ? 'checked' : '' }}>
              <span class="text-sm">Freestanding</span>
            </label>
          </div>
        </div>
      </div>

      @error("accessories.$a->id.size")  <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
      @error("accessories.$a->id.type")  <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
    </div>
  @endforeach
</div> --}}
      ...
      {{-- Accessory Section --}}
{{-- <div class="p-4 mt-6 border rounded-2xl">
  <h3 class="mb-3 font-semibold">Accessory</h3>

  @php
    $accName = strtoupper(old('accessory_name', $product->accessory_name ?? ''));
    $names   = ['MICROWAVE','WASHING MACHINE','DISHWASHER','OVEN','FRIDGE','FREEZER','DRYER'];
    $accSize = old('accessory_size', $product->accessory_size ?? '');
    $sizes   = ['45cm','50cm','54cm','60cm','70cm','80cm','90cm'];
    $accType = old('accessory_type', $product->accessory_type ?? '');
  @endphp
 --}}
  {{-- 1) Accessory Name (labels as radio pills) --}}
  {{-- <fieldset class="mb-4">
    <legend class="block mb-2 text-sm font-medium text-gray-700">Accessory Name</legend>
    <div class="flex flex-wrap gap-2">
      @foreach($names as $n)
        <label class="cursor-pointer">
          <input
    ...
    @error('accessory_name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
  </fieldset>

  <div class="grid grid-cols-1 gap-4 md:grid-cols-3"> --}}
    {{-- 2) Size (select) --}}
    {{-- <div class="md:col-span-1">
      <label class="block mb-1 text-sm font-medium text-gray-700">Accessory Size</label>
      <select name="accessory_size" class="w-full px-3 py-2 border rounded-lg" required>
        <option value="">Select size</option>
        @foreach($sizes as $s)
          <option value="{{ $s }}" {{ $accSize === $s ? 'selected' : '' }}>{{ $s }}</option>
        @endforeach
      </select>
      @error('accessory_size') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
    </div> --}}

    {{-- 3) Product Type (radio) --}}
    {{-- <div class="md:col-span-2">
      <label class="block mb-1 text-sm font-medium text-gray-700">Product Type</label>
      <div class="flex gap-4 items-center h-[42px]">
        <label class="inline-flex items-center gap-2 cursor-pointer">
          <input type="radio" name="accessory_type" value="Inbuilt"
                 {{ $accType === 'Inbuilt' ? 'checked' : '' }} required>
          <span class="text-sm">Inbuilt</span>
        </label>
        <label class="inline-flex items-center gap-2 cursor-pointer">
          <input type="radio" name="accessory_type" value="Freestanding"
                 {{ $accType === 'Freestanding' ? 'checked' : '' }} required>
          <span class="text-sm">Freestanding</span>
        </label>
      </div>
      @error('accessory_type') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
    </div>
  </div>
</div> --}}
