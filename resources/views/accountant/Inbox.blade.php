    ...
    <main class="ml-64 mt-[70px]   flex-1 bg-gray-100   items-center">
        ...
        <div class="flex h-screen">
            ...
            <div class="w-1/3 pt-4 overflow-y-auto bg-[#FCFDFF]">
                ...
                <div class="pt-2 pl-4 pr-4 mt-4 mb-6 ml-7 mr-7">
                    ...
                    <input type="text" placeholder="Search messages"
                        class="items-center w-full px-3 py-2 border-b border-gray-200 rounded-[10px]" />
                </div>
                @foreach ($users as $user)
                    <a href="{{ route('accountant.inbox', $user->id) }}"
                        ...
                        <ul>
                            <li
                                class="flex items-center px-4 py-3 space-x-3 border-b cursor-pointer hover:bg-yellow-50 ">
            ...
            <div class="flex  flex-col justify-between w-3/4 bg-[#F9F7F7]">
                <div id="chat"
                    class="sticky top-[100px] left-0 z-30 flex items-center justify-between px-6 py-4 bg-white border-b">
                    <div>
                ...
                <div id="chat-box" class="flex-1  overflow-y-auto bg-[#F9F7F7]">
                    @if ($messages)
                        @foreach ($messages as $message)
                            <div
                                class="mb-2 ml-4 mt-4 text-sm rounded-[9px]  {{ $message->sender_id == auth()->id() ? 'text-right' : 'text-left' }}">
                                <div
                                    class="inline-block p-2 rounded-lg
                            {{ $message->sender_id == auth()->id() ? 'bg-fuchsia-900 text-white ml-50' : 'bg-[#fff] text-black' }}">
                                    {{ $message->message }}
                                    <span class="text-[11px] ml-[5px] text-gray-500">
                ...
                @if ($userId)
                    <form id="message-form" class="sticky bottom-0 left-0 z-30 flex p-4 border-t ">
                        ...
                        <input type="text" id="message" class="flex-1 p-2 border-b border-gray-200 rounded-[10px]"
                            placeholder="Write your message here...">
                        <button type="submit"
                            class="px-4 py-2 ml-2 text-white  bg-fuchsia-900 hover:bg-yellow-600 rounded-[10px]">Send</button>
                    </form>
                @endif
            </div>
    </main>
                ...
                fetch(`/accountant/inbox/fetch/${receiverId}`)
                    ...
                    .then(data => {
                        ...
                        data.forEach(msg => {
                            ...
                            div.classList.add('mb-2', 'flex', 'text-sm', 'ml-4', 'mt-4', msg
                                .sender_id == {{ auth()->id() }} ? 'justify-end' : 'justify-start'
                                );
