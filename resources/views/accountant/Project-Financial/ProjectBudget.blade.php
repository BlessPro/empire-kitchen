    <section class="p-3 space-y-4 sm:p-4">
        ...
        <div class="bg-white shadow-md rounded-[20px] overflow-hidden">
            <div
                ...
                <div>
                    <h1 class="text-lg font-semibold text-gray-900 sm:text-xl">Budgets</h1>
                    <p class="text-sm text-gray-500">Review budgets, their periods, and spending.</p>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm text-left text-gray-600">
                    <thead class="text-gray-700 bg-gray-100">
                        <tr>
                            <th class="px-4 py-3 font-semibold">Budget Name</th>
                    ...
                    <tbody class="divide-y">
                        @forelse ($budgets as $budget)
                            @php
                                ...
                                $tone = $budget->budget_status['tone'] ?? 'gray';
                                $badge = match ($tone) {
                                ...
                                $start = optional($budget->start_date)->format('M j, Y') ?? '-';
                                $end = optional($budget->end_date)->format('M j, Y') ?? '-';
                            @endphp
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">{{ $currency }} {{ $formatAmount($budget->balance ?? 0) }}</td>
                                <td class="px-4 py-3">
                                    <span class="inline-flex px-4 py-[3px] text-xs font-medium border rounded-full {{ $badge }}">
                                        {{ $budget->budget_status['label'] ?? 'No budget' }}
                                    </span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-6 text-center text-gray-500">No budgets found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
