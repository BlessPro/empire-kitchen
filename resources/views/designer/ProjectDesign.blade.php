                    ...
                    <div x-data="projectDropdown()" class="relative w-full mb-4">
                        ...
                        <div class="relative">
                            <input type="text" x-model="search" @focus="open = true" @click="open = true"
                                @input="watchSearch()" @keydown.escape="open = false"
                            ...
                            <button type="button" @click="clearSelection" x-show="selectedId || search"
                            ...
                            <ul x-show="open" @click.away="open = false"
                                ...
                                <template x-for="(project, index) in filteredProjects" :key="project.id">
                                    <li :class="{ 'bg-blue-100': index === focusedIndex }" @click="selectProject(project)"
                                        @mouseenter="focusedIndex = index"
                                        class="px-4 py-2 cursor-pointer hover:bg-blue-100" x-text="project.name"></li>
                                </template>
                                <li x-show="filteredProjects.length === 0" class="px-4 py-2 text-gray-500">No projects
                                    found.</li>
                            </ul>

                            <!-- Hidden input (kept here for your UI logic) -->
                            <input type="hidden" id="project_id_dropdown_hidden" name="project_id_dropdown"
                                :value="selectedId">
                        </div>
                    </div>

                    {{-- ========== FORM (kept) ========== --}}
                    <form id="designUploadForm" action="{{ route('design.store') }}" method="POST"
                        ...
                        <div
                            class="border-dashed border-2 border-gray-300 p-[40px] text-center rounded-[10px] mb-5  bg-white">
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="6" placeholder="Start typing here"
                                class="w-full border-gray-200 rounded-[10px] px-4 py-2 bg-white"></textarea>
                        </div>

                        <div class="flex items-center justify-center mx-auto mt-4 text-center">
                            <button
                                class="bg-fuchsia-900 hover:bg-purple-800 text-white px-6 py-2 rounded-[10px] flex items-center justify-center space-x-2"
                                type="submit">
    ...
    <script>
        ...
        function projectDropdown() {
            return {
                ...
                focusNext() {
                    if (this.focusedIndex < this.filteredProjects.length - 1) this.focusedIndex++;
                },
                focusPrev() {
        ...
        (function() {
            ...
            function init() {
                ...
                if (invoiceCancelBtn) invoiceCancelBtn.addEventListener('click', () => {
                    hideFlex(invoiceModal);
                    window.location.reload();
                });
                if (invoiceCloseBtn) invoiceCloseBtn.addEventListener('click', () => {
                    hideFlex(invoiceModal);
                    window.location.reload();
                });
                ...
                function startPrepProgress(fileList) {
                    ...
                    if (prepRafId) {
                        cancelAnimationFrame(prepRafId);
                        prepRafId = null;
                    }

                    // Update filename + meta
                    if (fileNameLbl) fileNameLbl.textContent = fileList.length === 1 ? fileList[0].name :
                        `${fileList.length} files`;
                    if (progressMeta) {
                        const totalSizeMB = (Array.from(fileList).reduce((s, f) => s + (f.size || 0), 0) / (1024 *
                            1024)).toFixed(1);
                        progressMeta.textContent =
                            `${fileList.length} file${fileList.length > 1 ? 's' : ''} • ${totalSizeMB} MB`;
                    }
                    ...
                    const duration = Math.min(2000, 800 + Math.round(totalSizeMBRaw *
                    150)); // 0.8s + ~150ms/MB (cap 2s)
