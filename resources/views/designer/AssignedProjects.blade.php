                            ...
                            <tbody class="text-gray-700 border-b divide-y divide-gray-100 hover:bg-gray-50">
                                @forelse ($projects as $project)
                                    <tr class="border-b hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-2 text-[13px]">
                                            @php
                                                $d = $project->latestInstallation?->install_date;
                                            @endphp
                                            {{ $d ? ($d->format('H:i:s') === '00:00:00' ? $d->format('d M Y') : $d->format('d M Y A g:i A')) : '-' }}
                                        </td>
                                        <td class="px-4 py-2 text-[13px]">
                                            @php
                                                $status = (string) ($project->status ?? '-');
                                            @endphp
                                            <span class="px-3 py-1 text-[13px] {{ $statusClasses[$status] ?? $defaultClass }}">
                                                {{ $status }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-2 text-[13px] text-left">
                                            <a href="{{ route('designer.projects.info', $project->id) }}"
                                               class="inline-flex items-center text-[13px] justify-center w-8 h-8 rounded-md hover:bg-gray-100"
                                               title="View">
                                               <iconify-icon icon="mdi:eye-outline" class="text-[13px]"></iconify-icon>
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-4 py-6 text-center text-gray-500">No assigned projects yet.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
