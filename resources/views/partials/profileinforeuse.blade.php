...
<x-dynamic-component :component="$layoutComponent">
    ...
    @php
        ...
        // Shared product helpers (fallback to first product if none marked active)
        $products = collect($project->products ?? []);
        $activeProduct = $products->firstWhere('active', true) ?? $products->first();
        $activeProductId = $activeProduct['id'] ?? null;
        $activeProductName = $activeProduct['name'] ?? ($activeProductId ? 'Product #' . $activeProductId : null);
    ...
    <div class="p-4 sm:p-6 bg-[#F9F7F7] min-h-screen">
            ...
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    @php
                        $pmRoute = $projectManagementRoute
                            ?? (auth()->user()?->role === 'tech_supervisor'
                                ? 'tech.ProjectManagement'
                                : 'admin.ProjectManagement');
                    @endphp
                    <a href="{{ Route::has($pmRoute) ? route($pmRoute) : '#' }}">
                ...
                <!-- ADD CLIENT BUTTON -->
                {{-- Comments: Button + Drawer (brand color override for send button) --}}
                <div class="admin-comments">
                    <style>
                        .admin-comments .bg-emerald-600 { background-color: #5A0562 !important; }
                        .admin-comments .hover\:bg-emerald-700:hover { background-color: #4a044c !important; }
                        .admin-comments .hover\:bg-emerald-800:hover { background-color: #4a044c !important; }
                        .admin-comments footer form button[type="submit"]:not([disabled]) {
                            background-color: #5A0562 !important; color: #fff !important;
                        }
                        .admin-comments footer form button[type="submit"]:not([disabled]):hover {
                ...
                <details open class="group">
                    ...
                    <div class="px-5 pb-4">
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
                            ...
                            @php
                                $projectIdForRoutes = data_get($project, '__meta.project_id', $project->id ?? null);
                            @endphp
                            @foreach (($project->phases ?? []) as $p)
                                <label class="flex items-start gap-3 px-3 py-2 border border-gray-200 rounded-lg">
                                    <input type="checkbox"
                                        ...
                                        data-phase="{{ \Illuminate\Support\Str::slug($p['name'] ?? '', '-') }}"
                                        data-url="{{ route('admin.projects.phases.toggle', [
                                            'project' => $projectIdForRoutes,
                ...
                <div class="px-5 py-4">
                    ...
                    <div class="mt-3 border border-gray-200 rounded-xl">
                        ...
                        <div class="flex flex-wrap items-center gap-2 px-4 py-3 text-sm">
                            <span id="finish-label"
                                class="{{ $chip }}">{{ $project->finish['label'] ?? '—' }}</span>
                            <span class="{{ $chip }}">Color: <span
                                    class="ml-1 font-medium">{{ $project->finish['color'] ?? '—' }}</span></span>
                            @if (!empty($project->finish['color'] ?? null))
                                <span id="finish-color" class="inline-flex w-8 h-5 rounded-md ring-1 ring-gray-300"
                                    style="background: {{ e($project->finish['color']) }}"></span>
                            @else
                                <span id="finish-color"
                                    class="inline-flex hidden w-8 h-5 rounded-md ring-1 ring-gray-300"></span>
                            @endif

                            @if (!empty($project->finish['image_url'] ?? null))
                            ...
                            @else
                                <span id="finish-image"
                                    class="hidden inline-block w-10 h-5 bg-center bg-cover rounded-md ring-1 ring-gray-300"></span>
                            @endif
                        </div>
                    ...
                    <div class="mt-3 border border-gray-200 rounded-xl">
                        ...
                        <div class="flex flex-wrap items-center gap-2 px-4 py-3 text-sm">
                            <span id="worktop-label"
                                class="{{ $chip }}">{{ $project->worktop['label'] ?? '—' }}</span>

                            @if (!empty($project->worktop['color'] ?? null))
                                <span class="{{ $chip }}">Color: <span
                                        class="ml-1 font-medium">{{ $project->worktop['color'] }}</span></span>
                                <span id="worktop-color" class="inline-flex w-8 h-5 rounded-md ring-1 ring-gray-300"
                            ...
                            @if (!empty($project->worktop['image_url'] ?? null))
                    ...
                    <div class="mt-3 border border-gray-200 rounded-xl">
                        ...
                        <div class="flex flex-wrap items-center gap-2 px-4 py-3 text-sm">
                            <span id="sink-bowl-label"
                                class="{{ $chip }}">{{ $project->sink_tap['bowl'] ?? '—' }}</span>

                            @if (!empty($project->sink_tap['color'] ?? null))
                            ...
                            <span id="sink-handle-label"
                                class="{{ $chip }}">{{ $project->sink_tap['handle'] ?? '—' }}</span>

                            @if (!empty($project->sink_tap['image_url'] ?? null))
                                <span id="sink-image"
                                    class="inline-block w-10 h-5 bg-center bg-cover rounded-md ring-1 ring-gray-300"
  ...
  Edit accessories
    ...
    {{-- enforce sequential phase toggles --}}
    <script>
        (function() {
            const order = [
                'work-order',
            ...
            const toggles = Array.from(document.querySelectorAll('.phase-toggle'));
            ...
            const refreshLocks = () => {
                const ordered = toggles
                    .filter(cb => orderIndex(cb.dataset.phase) < Infinity)
                    .sort((a, b) => orderIndex(a.dataset.phase) - orderIndex(b.dataset.phase));
                ...
                // leave phases not in the sequence unrestricted
                toggles
                    .filter(cb => orderIndex(cb.dataset.phase) === Infinity)
