                   ...
                   <div class="flex items-center justify-between mb-6">
                       <div class="flex items-center justify-between mb-6">
                           ...
                           <a href="{{ route('tech.ClientManagement') }}">
                               <h3 class="font-sans font-normal text-black cursor-pointer hover:underline">Clients
                                   Management</h3>
                           </a>

                           <span><i data-feather="chevron-right"
                                   class="w-[4] h-[3] text-fuchsia-900 ml-[3px]"></i></span>
                           </h3>

                           <span><span
                                   class="mr-[12px] font-normal text-black-900">{{ $project->client->title . ' ' . $project->client->firstname . ' ' . $project->client->lastname }}</span>
                           </span>
                           <span><i data-feather="chevron-right"
                                   class="w-[4] h-[3] text-fuchsia-900 ml-[3px]"></i></span>
                       ...
                       <div class="p-6 bg-white shadow lg:col-span-2 rounded-xl">
                           ...
                           <div class="flex items-center justify-between mb-6">
                               <div class="flex items-center justify-between mb-6">
                                   ...
                                   <span
                                       onclick="openStatusModal('{{ $project->id }}', '{{ $project->status }}', event)"
                           ...
                           <div class="mt-6 space-y-4 text-sm text-gray-700">
                                   ...
                                   <div x-data="{
                                       ...
                                       <div x-show="showCalendar" x-transition @click.away="showCalendar = false"
                                           ...
                                           <div class="text-center font-semibold text-lg mb-2 text-gray-800">
                                               <span
                                                   x-text="new Date(tempDate).toLocaleString('default', { month: 'long', year: 'numeric' })"></span>
                                           </div>

                                           <input type="date" x-model="tempDate"
                               ...
                               <div class="flex items-center gap-2">
                                   <iconify-icon icon="famicons:location-outline" width="24"
                                       style="color: #5A0562;"></iconify-icon>

                                   <span class="font-medium">Address:</span>
                                   <span><span
                                           class="mr-[12px] font-normal text-black-900">{{ $project->location }}</span>
                                   </span>
                               </div>
                               ...
                               <div class="flex items-center gap-2">
                                   ...
                                   <span><span
                                           class="mr-[12px] font-normal text-black-900">{{ $project->techSupervisor->name ?? 'Not Assigned' }}</span>
                                   </span>
                           ...
                           <div class="mt-6">
                               <h3 class="mb-2 text-sm font-semibold text-gray-800">Measurement (in length, width,
                                   height)</h3>
                               <div class="flex gap-8">
                                   @forelse ($project->measurement as $measurement)
                                       <div class="flex items-center gap-1">

                                           <iconify-icon icon="solar:ruler-outline" width="22"
                                               style="color: #5A0562;"></iconify-icon>
                                           <span>
                                               <p> {{ $measurement->length }}mm</p>
                                           </span>
                                       </div>
                                       <div class="flex items-center gap-1">
                                           <iconify-icon icon="solar:ruler-outline" width="22"
                                       ...
                                       <div class="flex items-center gap-1">
                                           ...
                                           <span>
                                               <p> {{ $measurement->height }}mm</p>
                                           </span>
                                       </div>
                                   @empty
                                       <p onclick="window.location='{{ route('tech.CreateMeasurement') }}'"
                                           class="ml-50 font-semibold text-[13px] cursor-pointer text-fuchsia-900 hover:text-[#F59E0B]">
                                           + Add measurement</p>
                                   @endforelse
                                                           ...
                                                           <li class="flex items-center justify-between">
                                                               <div class="flex items-center gap-3">
                                                                   ...
                                                                   <div>
                                                                       <p class="font-medium text-gray-800">
                                                                           {{ basename($image) }}</p>
                                                                       <p class="text-xs text-gray-500">Uploaded on
                                                               ...
                                                               <div class="flex gap-4 text-purple-600">
                                                                   <a href="{{ asset('storage/' . $image) }}"
                                                                       target="_blank">
                                                                       <i data-feather="eye"
                                                                           class="w-5 h-5 text-fuchsia-900"></i>
                                                                   </a>
                                                                   <a href="{{ asset('storage/' . $image) }}"
                                                                       download>
                                           ...
                                           @if (!empty($design->images) && is_array($design->images))
                                               ...
                                               <div class="w-full mx-auto ">
                                                   ...
                                                   @foreach ($design->images as $image)
                                                       <!-- Designs -->
                                                       <div class="pt-4 mt-6 border-t">
                                                           <h3 class="mb-2 text-sm font-semibold text-gray-900">Designs
                                                           </h3>
                                                           <ul class="">
                                                               <li class="flex items-center justify-between">
                                                                   <div class="flex items-center gap-3">
                                                                   ...
                                                                   <div class="flex gap-4 text-purple-600">
                                                                       <button
                                                                           onclick="{{ route('designer.ProjectDesign') }}">
                       ...
                       <div class="flex flex-col justify-between p-6 bg-white shadow rounded-xl">
                                           ...
                                           <div>
                                               <p class="text-sm font-semibold text-gray-800">
                                                   {{ $comment->user->name }} <span
                                                       class="text-xs font-normal text-gray-400">{{ $comment->user->created_at->diffForHumans() }}</span>
                                               </p>
                           ...
                           <div class="mt-6">
                               ...
                               <form method="POST" id="commentForm"
                                   action="{{ route('project.comment.store', $project->id) }}">
                                   @csrf
                                   <textarea name="comment" placeholder="Start typing"
                                       class="w-full px-4 py-2 text-sm border rounded focus:outline-none focus:ring-2 focus:ring-purple-500"></textarea>
                                   <button type="submit"
                                       class="px-4 py-2 mt-2 text-white rounded bg-fuchsia-900">Post Comment</button>
                               </form>
                           </div>
                       </div>
                       ...
                       <form id="statusForm">
                           ...
                           <div class="flex justify-between">
                               <button type="button" onclick="closeStatusModal()"
                                   class="text-gray-500 font-medium hover:underline">Cancel</button>
                               <button type="submit"
                                   class="bg-fuchsia-900 text-white rounded-full px-5 py-1.5">Confirm</button>
                           </div>
                       </form>
                   </div>
